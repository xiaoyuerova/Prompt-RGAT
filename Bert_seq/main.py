import os.path
import torch
from bert_seq import *
from bert_seq.utils import load_src_for_train, set_seed, load_pkl, save_pkl, get_logger
...
def main(args: Configs):
    ...
    if args.from_begin:
        ...
        save_pkl(train_data, os.path.join(args.output_dir, args.model_file_name + '_train_data.pkl'))
        save_pkl(dev_data, os.path.join(args.output_dir, args.model_file_name + '_valid_data.pkl'))
        save_pkl(test_data, os.path.join(args.output_dir, args.model_file_name + '_test_data.pkl'))
    else:
        train_data = load_pkl(os.path.join(args.output_dir, args.model_file_name + '_train_data.pkl'))
        dev_data = load_pkl(os.path.join(args.output_dir, args.model_file_name + '_valid_data.pkl'))
        test_data = load_pkl(os.path.join(args.output_dir, args.model_file_name + '_test_data.pkl'))

    log.debug("Building model...")
    model_file = os.path.join(args.output_dir, args.model_file_name)
    model = BertSeq(args).to(args.device)
...
if __name__ == "__main__":
    config = Configs(from_begin=True,
                     ...
                     model_file_name='seqSdzPisaTest.pt',
                     batch_size=128,
                     learning_rate=5e-5,
                     seed=1,
