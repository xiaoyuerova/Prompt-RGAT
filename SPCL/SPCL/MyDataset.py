...
class MyDataset(Dataset):
    ...
    def init_data_with_context(self, src: list):
        # log.info('init_data_with_context...')
        if self.tmp_name:
        ...
        for dialogue in src:
            ...
            for idx, turn_data in enumerate(dialogue):
                ...
                query_idx = idx
                prompt = 'the cooperative problem-solving skills {} has shown is <mask>'.format(turn_data.speaker)
                ...
                if self.train and idx > 3:
                    for query_idx in range(lidx, idx):
        ...
        log.debug("train: {}, Data check: {}".format(self.train, data_check[:10]))
        # log.info('init_data_with_context completed')
