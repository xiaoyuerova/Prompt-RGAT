...
class EdgeAtt(nn.Module):
    ...
    def __init__(self, g_dim,
        ...
        self.weight.data.normal_(0, var)        # 正态分布（normal distribution）
    ...
    def forward(self,
        ...
        :param node_features:
        :param text_len_tensor:
        :param edge_ind:
        :return: [B, cur_len, g_dim]
        """
        batch_size, mx_len = node_features.size(0), node_features.size(1)
        alphas = []

        weight = self.weight.unsqueeze(0).unsqueeze(0)
        att_matrix = torch.matmul(weight, node_features.unsqueeze(-1)).squeeze(-1)  # att_matricx: [B, L, D_g]
        for i in range(batch_size):
            cur_len = text_len_tensor[i].item()
            alpha = torch.zeros((mx_len, int(text_len_tensor.sum().item()))).to(self.device)
