...
def batch_graphify(
        ...
        device):
    """
    将一个batch内的图合并。并计算node_features, edge_index, edge_norm, edge_type, edge_index_lengths
    :param features: padding过的tensor
    :param lengths:
    :param speaker_tensor:
    :param wp:
    :param wf:
    ...
    length_sum = 0
    edge_ind = []
    edge_index_lengths = []         # 将batch中每个序列中的节点合并到一起，重新编号
    ...
    for j in range(batch_size):
        ...
        # 图合并
        # item是单个序列内边节点编号；item_rec是合并后序列内边节点编号
        for item, item_rec in zip(perms, perms_rec):
            ...
            # edge_type.append(edge_type_to_idx[c])
...
def edge_perms(
        length: int,
        window_past: int = -1,
        window_future: int = -1) -> list[tuple]:
    """
    构建图的边
