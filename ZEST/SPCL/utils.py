...
def load_src_00(file_name, args: Configs, replace_name=True, shuffle=False) -> list[list[DataType]]:
    if not replace_name:
...
def load_src_for_predict(data, args: Configs, file_type='csv', shuffle=False):
    """
    ...
    if args.src_type == 'mdla_pisa':
        data = data[['group_number', 'student_name', 'data2', 'data1', 'label']]
        src = []
        for team, group in data.groupby('group_number'):
            seq = []
            for index in group.index:
                seq.append(DataType(**{
                    'speaker': group['data2'][index],
                    'text': group['data1'][index],
...
def gen_all_reps(model, data):
    ...
    with torch.no_grad():
        ...
        return idxes, results, label_results
